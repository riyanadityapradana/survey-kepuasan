...
$jenis = 'ralan';
$judul = 'Hasil Survei Rawat Jalan';
$formUrl = url('survey/form_ralan.php');
$exportUrl = url('admin/hasil/export_ralan.php');
$activeMenu = 'ralan';
$pageTitle = $judul;
...
$returnUrl = url('admin/hasil/ralan.php') . query_string();
...
$kategoriOptions = [];
$kategoriQuery = mysqli_query($conn, "SELECT DISTINCT kategori FROM pertanyaan WHERE jenis = 'ralan' ORDER BY kategori ASC");
...
if ($respondentIds) {
    ...
    $answerSql = "SELECT r.id AS id_responden, p.kategori, p.pertanyaan, j.nilai FROM responden r INNER JOIN jawaban j ON j.id_responden = r.id INNER JOIN pertanyaan p ON p.id = j.id_pertanyaan WHERE r.id IN ($idList) AND p.jenis = 'ralan'";
...
function badge_detail_ralan(string $jawaban): string
{
        ...
        <div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    ...
                    <a href="<?= $formUrl; ?>" class="btn btn-outline-primary">Buka Form Ralan</a>
                </div>
            </div>
            ...
            <form method="get" class="filter-grid mb-4">
                ...
                <div class="filter-actions"><button type="submit" class="btn btn-rs-primary"><i class="fa-solid fa-filter me-2"></i>Terapkan</button><a href="<?= url('admin/hasil/ralan.php'); ?>" class="btn btn-outline-secondary">Reset</a></div>
            </form>
                        ...
                        <?php else : ?>
                            <?php foreach ($respondents as $detail) : ?>
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex gap-2 flex-wrap">
                                            <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#detailRalan<?= $detail['id']; ?>"><i class="fa-solid fa-eye me-1"></i>Detail</button>
                                            <?php if (user_punya_role('admin')) : ?>
                                                ...
                                                <?php endif; ?>
                                                <a href="<?= url('admin/hasil/hapus_responden.php?id=' . $detail['id'] . '&jenis=ralan&return=' . urlencode($returnUrl)); ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Hapus responden ini beserta semua jawabannya?');"><i class="fa-solid fa-trash me-1"></i>Hapus</a>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                </tr>
...
<?php foreach ($respondents as $detail) : ?>
    <div class="modal fade" id="detailRalan<?= $detail['id']; ?>" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-scrollable modal-dialog-centered">
            <div class="modal-content border-0 google-card">
                <div class="modal-header border-0 pb-0"><div><h5 class="modal-title fw-bold mb-1">Detail Jawaban Responden</h5><p class="text-muted small mb-0">Rawat Jalan</p></div><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body pt-3">
                    ...
                    <div class="detail-list">
                        <?php foreach ($detail['jawaban'] as $kategoriNama => $items) : ?>
                            <div class="detail-group"><div class="section-heading mb-3"><h6 class="fw-semibold mb-1"><?= e($kategoriNama); ?></h6><p class="text-muted small mb-0">Daftar jawaban pada kategori ini.</p></div><?php foreach ($items as $item) : ?><div class="detail-list-item"><div class="detail-list-head"><div class="flex-grow-1"></div><?= badge_detail_ralan($item['jawaban']); ?></div><p class="detail-pertanyaan mb-0"><?= e($item['pertanyaan']); ?></p></div><?php endforeach; ?></div>
                        <?php endforeach; ?>
                    </div>
                </div>
            </div>
